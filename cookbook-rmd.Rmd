...
* **A `docs/` folder**, which will be the folder that acts as the "boarding area" for your rendered site files.
...
2. **Open `about.Rmd` and add some new content.** This will be a second, distinct page of your site. This file can have any name, but we will stick to "about" for now.
...
4. **Click *Git* ** in the same RStudio pane that also contains the Environment tab.
...
**And here's what that looks like on a real site**:
