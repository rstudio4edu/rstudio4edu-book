...
knitr::include_graphics("images/illos/intro-distill.jpg")
```
...
knitr::include_graphics("images/illos/Caring-distill.jpg")
```
...
knitr::include_graphics("images/illos/Dressup-distill.jpg")
```
...
knitr::include_graphics("images/illos/Fancy-distill.jpg")
```
...
<center>![](images/illos/intro-distill.jpg){width=70%}</center>
...
```{r distill-site-dir, eval = FALSE, fig.cap = "Distill site skeleton", out.width = "40%"}
...
1. **`_site/` folder**: This is the default output folder, where knitted versions of our `.Rmd`s live once they've been processed to become `.html` files. We're eventually going to replace this folder with a `docs/` folder instead so we can host our site on GitHub pages--we'll set that up soon. 
...
## Change output directory {#distill-site-output}

Now let's make a small change. In order to publish to GitHub Pages, we'll want to send all the rendered site files (i.e., all those ending in `.html`) to our "boarding area", which is the `docs/` folder. We need to edit the `_site.yml` to do this. This step is once-per-website step. 

**Open `_site.yml`** and change the `output_dir:` to be `docs`:
...
## Turn off Jekyll {#distill-site-nojekyll}

This bit is only necessary if you plan to use GitHub Pages for publishing your Distill website. We need to tell GitHub Pages to bypass using Jekyll to build your site. We do this by adding a single empty file named `.nojekyll` to your project root directory.
...
1. **Scroll down and select** *Distill Website*
...
1. Then follow all of our instructions above starting at [changing the site output directory](#distill-site-output).
