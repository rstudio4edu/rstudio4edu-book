...
In broad strokes, a bookdown site is created when several `.Rmd` files get merged together (in a particular order--but more on that later), resulting in a single (sometimes massive) `.Rmd` file. This monolith of an `.Rmd` then gets rendered into website-ready, HTML form. What is sometimes unintuitive (especially for those coming from R Markdown sites) is that new pages are determined by the level 1 headers (i.e. headers made with a single `#`) in your content--not necessarily by distinct `.Rmd` documents in your project.  
...
* How to use bookdown features, e.g. crossreferencing, as you create your content
...
<center>![](images/illos/intro-book.jpg){width=70%}</center>
...
* **Option 2**: *From the R console*, you can run: 
