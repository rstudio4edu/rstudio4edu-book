...
<center>![](images/illos/Fancy-book.jpg){width=40%}</center>

Now it's time to customize our book even further. We can do a little bit of HTML and CSS work to add-on some fancy features, so you might want to check out our [CSS crash course]{#css-crash} if this is your first time dipping your toe into the water. 
...
* For organization purposes, we recommend creating a folder called `assets/` in your project directory and moving the CSS file called `style.css` that came with your skeletal book into that folder.
* AND, if you just followed the previous suggestion, then you'll also want to update your `_output.yml` file to show this change `css: assets/style.css`.
...
:::design
We recommend a sans-serif font (i.e. no little feet on the letters) because it's easier to read at smaller sizes on screens compared to serif fonts. Don't pick anything too narrow, too wide, or ornate.)
:::
...
3. Click on the `CUSTOMIZE` tab, and select the extra font weights and styles you want. Don't check any that you don't think you'll use. In addition to regular, we're choosing one italic and one bold. 
...
1. On the very first line paste in the Google Font import code for the Muli font. Don't include the `<style>` tags. This makes the font available within your bookdown site.
...
:::tip
It is also perfectly legitimate to import your font by embedding the script (with `<script>` tags) in an HTML file. But if you're beginning, it's easier to just keep all the font stuff together in the CSS file. 
:::


#### Style your text

Now we can use this font to style our body's text by using the `font-family:` attribute. 

1. Add the following to the CSS. This code in the curly brackets came from that second code chunk on the Google Fonts page:
    ...
    .book.font-family-1 {
     ...
     ```
By the way, a little insight in the the trial-and-error process of getting the right selectors: I first tried using `body` as the selector here, but it didn't work. I would have *never* figured out that this selector would work by by using the tips we suggested in the [CSS crash course](#css-crash) to use the inspector and start hovering elements. So, `.book.font-family-1` is what happens to be what pulls the main text of the book. No magic to this, folks. Just a little grit!

:::tip
Organize your CSS with comments like `/* --insert-comment-- */` to create make-shift section dividers for your CSS, as we've done above.
:::

#### Confirm `style.css` is linked in `_output.yml`

If you've been following along using the book skeleton, then you are all set, and there's nothing for you to do here. But just for completeness, we'll point out that your all your hard CSS work will only be applied if your book knows about it. And it will know if you reference `style.css` in `_output.yml`:

    ```{r, echo = FALSE}
    yml(author = FALSE, date = FALSE) %>% 
        yml_output(bookdown::gitbook(css = "assets/style.css")
        ) %>% 
      yml_pluck("output") %>% 
...
:::design
Header fonts can have some more pizazz `r emo::ji("sparkles")`, but stay away from the intense fonts like handwritings or very artsy looking things, unless you're only applying it to the book title for dramatic effect.
:::

2. Optionally, we also change the `color:` and `font-size:` for the title and headers. 
...
You can add interest to text on your page without being distracting by choosing very dark flavors of your accent color as an alternative to choosing black. For example, when using a [color picker](https://htmlcolorcodes.com/color-picker/), slide down to the darkest, *almost*-black-but-not-quite version of your color of choice. Consider using this for your body text's or headers' color.
...
* A logo file, better if it's a PNG with a transparent background.
* Edit `_output.yml`
...
3. **Open `style.css`**. 
4. **Add** the following to your CSS. This will make space for your logo image above your TOC.
    ...
    ```
5. **Build** and preview your handywork. 
6. Go back to the CSS and adjust the `width:` parameter of `.toc-logo` to adjust the size of your logo.
...
1. **Paste** these contents into the file.
...
1. **Replace the file path** to the image with your own.

1. **Save** this file in your project directory, ideally inside of the `assets/` folder. Name it `hero-image.html`. You can choose a different name. 


#### Add the HTML file to `_output.yml`

Now we tell bookdown that this HTML exists and that we want its contents (our image) to be the *first* thing in the `<body>` tag:
...
1. **Add** the `includes:` and `before_body:` options with a path to your `hero-image.html` file.
    ...
    yml(author = FALSE, date = FALSE) %>% 
        yml_output(bookdown::gitbook(
          includes = includes2(before_body = "assets/hero-image.html"),
          css = "style.css")
        ) %>% 
      yml_pluck("output") %>% 
      asis_yaml_output(fences = FALSE) 
...
This last aesthetic adjustment goes out to everyone who notices the little things. 
Before we added a logo, you may have noticed that the horizontal line from the toolbar and the TOC did not match up perfectly. This CSS fixes that:
...
1. Save this file as `book.js`. (You can give it another name, but the convention is usually to name the Javascript file after the project you're using it for.) Save this file in your working directory or in the `assets/` folder.  
...
1. Reference your HTML file with the `includes: in_header:` option underneath `bookdown::gitbook`. Make sure CSS file is also referenced. This part of your `_output.yml` should look something like this:
    ...
    yml(author = FALSE, date = FALSE) %>% 
        yml_output(bookdown::gitbook(
          includes = includes2(in_header = "assets/header.html", before_body = "assets/hero-image.html"),
          css = "style.css")
        ) %>% 
      yml_pluck("output") %>% 
      asis_yaml_output(fences = FALSE) 
...
* **A little javascript** in a .js file (we can add on to the .js file we used to make the anchor links).
...
1. Add the javascript: paste the following into into the `book.js` file located in the `assets/` folder. 
...
1. Open `style.css` and add the following to style the appearance of the button:
...
1. The `border-right: none` parameter removes the line that separates the TOC by default. If you want to keep that, then just remove this. Or alternatively, style it differently by replacing `none` with something like `5px #5699cc solid`, for example.
...
1. To **change the TOC font**, paste in the CSS below. You'll have to choose from a font that you already imported [earlier](#book-font). 
...
2. We added **`margin-top: 1em`** for some much needed space above each Part title.  
3. We choose a **`color:`** that is similar to the dark blue of the links, but even closer towards black on that spectrum without going all the way to black (`#000000`).
4. By default, bookdown gives Part titles an opacity that makes them appear light-colored. We have to override this, by choosing an opacity of `1`. 
